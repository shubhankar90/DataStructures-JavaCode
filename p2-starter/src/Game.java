...
/**
 * Game is a controller that run a game in either interactive or batch mode.
 * Unless resized, the game is played on a board of size Constants.DEFAULT_SIZE.
 * 
 * Operations are provided to compare run times of different flood functions
 * (using reflection to automatically detect the defined methods in the Board
 * class).
 */
...
public class Game {
	...
	/**
	 * Runs a game in interactive mode. See comment for how to run in batch
	 * (i.e., testing) mode.
	 */
	...
	/**
	 * Creates either an interactive or simulated (autoplayed) game depending on
	...
	/**
	 * Processes one step of the game (where the player has selected the given
	 * color for their move) using the standard flood function.
	 */
	...
	/**
	 * Processes one stop of the game (where the player has selected the give
	 * color for their move) using the kth flood function (where k = 0, 1, 2,
	 * ...)
	 */
	...
	/**
	 * Runs a batch of tests, on boards of varying sizes, through autoPlay(),
	 * iterating over all defined flood functions, and then displays a graph of
	 * run times.
	 */
